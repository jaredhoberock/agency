/// \file
/// \brief Include this file to use default_bulk_then().
///
...
///
///
/// `default_bulk_then` is a control structure which asynchronously creates a group of function invocations with forward progress ordering as required by an execution policy.
...
/// The results of the continuation's invocations, if any, are collected into a container and returned as `default_bulk_then`'s asynchronous result.
/// A future object corresponding to the eventual availability of this container is returned as `default_bulk_then`'s result.
///
/// `default_bulk_then` asynchronously creates a group of function invocations of size `N`, and each invocation i in `[0,N)` has the following form:
///
...
/// The parameter `predecessor_arg` is a reference to the value of the future object used as a parameter to `default_bulk_then`. If this future object has a `void` value, then this parameter is omitted.
...
///
/// The difference between `default_bulk_then` and `bulk_then` is that unlike `bulk_then`, `default_bulk_then` is not a customization point whose
/// behavior can be customized with a fancy execution policy.
///
/// \param policy An execution policy describing the requirements of the execution agents created by this call to `default_bulk_then`.
...
///                    After `default_bulk_then` returns, `predecessor` is invalid if it is not a shared future.
...
///
/// The following example demonstrates how to use `default_bulk_then` to sequence a continuation after a predecessor task:
///
...
see_below
#endif
  default_bulk_then(ExecutionPolicy&& policy, Function f, Future& predecessor, Args&&... args)
{
