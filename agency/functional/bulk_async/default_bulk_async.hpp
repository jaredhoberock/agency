/// \file
/// \brief Include this file to use default_bulk_async().
///
...
///
///
/// `default_bulk_async` is a control structure which asynchronously creates a group of function invocations with forward progress ordering as required by an execution policy.
/// The results of these invocations, if any, are collected into a container and returned as `default_bulk_async`'s asynchronous result.
/// A future object corresponding to the eventual availability of this container is returned as `default_bulk_async`'s result.
///
/// `default_bulk_async` asynchronously creates a group of function invocations of size `N`, and each invocation i in `[0,N)` has the following form:
///
...
///
/// The difference between `default_bulk_async` and `bulk_async` is that unlike `bulk_async`, `default_bulk_async` is not a customization point whose
...
///
/// The following example demonstrates how to use `default_bulk_async` to create tasks which execute asynchronously with the caller.
///
...
see_below
#endif
  default_bulk_async(ExecutionPolicy&& policy, Function f, Args&&... args)
{
